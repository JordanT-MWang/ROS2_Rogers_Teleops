...
public:
  PDController()
  : Node("pid_controller_spring_node")
  {
    kp_values[0]= -5;
    kp_values[1]= -3;
    kp_values[2]= -5;
    ...
    sampling_time = 600/(pow(10.0,6.0));
    ...
    pwm_publisher_ = this->create_publisher<geometry_msgs::msg::Point>("pwm_values_spring", 10);
    timer_ = this->create_wall_timer(0.600ms, std::bind(&PDController::timer_callback, this));//change the ms seconds to match on the ardino code for best posting speeds
    //init subscripber to encoder values
    data_subscription_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
      "encoder_counter_spring", 10, std::bind(&PDController::sensor_topic_callback, this, _1));
    //init subcriber to reference angle
    reference_subscription_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
      "encoder_counter_gravity_delay", 10, std::bind(&PDController::reference_angles_topic_callback, this, _1));
    //pub delay values
    calc_delay_publisher_ =this->create_publisher<std_msgs::msg::Int64>(
      "calculated_delay_gravity_to_spring",10);

    //init subscriber to tuning p variables
    pid_p_tune_subscription_ = this->create_subscription<geometry_msgs::msg::Point>(
      "pid_tune_p_vars_spring", 10, std::bind(&PDController::pid_tune_p_vars_topic_callback, this, _1));
    //init subscriber to tuning i variables
    pid_i_tune_subscription_ = this->create_subscription<geometry_msgs::msg::Point>(
      "pid_tune_i_vars_spring", 10, std::bind(&PDController::pid_tune_i_vars_topic_callback, this, _1));
    //init subscriber to tuning d variables
    pid_d_tune_subscription_ = this->create_subscription<geometry_msgs::msg::Point>(
      "pid_tune_d_vars_spring", 10, std::bind(&PDController::pid_tune_d_vars_topic_callback, this, _1));
...
private:
  ...
  {
    reference_angles[0]= ref_msg->point.x*(270.0/199500.0);
    reference_angles[1]= ref_msg->point.y*(170.0/118550.0);
    reference_angles[2]= ref_msg->point.z*(190.0/141500.0);
    auto delay_msg = std_msgs::msg::Int64();
    delay_msg.data = (static_cast<long int>(std::chrono::system_clock::now().time_since_epoch().count()))
                          - (long int)(ref_msg->header.stamp.sec * std::pow(10,9) + ref_msg->header.stamp.nanosec);
  ...
  void sensor_topic_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
  {
    ...
    current_angles[0]=msg->point.x*(270.0/3030.0);
    current_angles[1]=msg->point.y*(150.0/2600.0);
    current_angles[2]=msg->point.z*(90.0/1075.0);
    /*
    ...
    */
    // for testing angles
    //auto pwm_values = geometry_msgs::msg::PointStamped();
  ...
  void timer_callback()
  {
    ...
    for(int joint_in_arm=0; joint_in_arm < num_joints; joint_in_arm++){
      ...
      controller_output[joint_in_arm] = kp_values[joint_in_arm]*error[joint_in_arm] + ki_values[joint_in_arm]*com_error[joint_in_arm]
                                        +kd_values[joint_in_arm]*(error[joint_in_arm]-prevous_error[joint_in_arm])/sampling_time;
  ...
  //shared PointStampeder for the timer, publisher, and subscriber
