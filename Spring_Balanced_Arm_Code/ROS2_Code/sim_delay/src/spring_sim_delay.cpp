...
class SimDelaySpring : public rclcpp::Node
{
public:
  SimDelaySpring()
  : Node("sim_delay_spring")
  {
    //pub enocder values 
    delay_publisher_ = this->create_publisher<geometry_msgs::msg::PointStamped>("encoder_counter_spring_delay", 10);

    //init subscripber to encoder values
    data_subscription_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
      "encoder_counter_spring", 10, std::bind(&SimDelaySpring::sensor_topic_callback, this, _1));
    //init subscriber to delay amount for just spring
    delay_subscription_ = this->create_subscription<std_msgs::msg::Int64>(
      "delay_sim_value_spring", 10, std::bind(&SimDelaySpring::delay_value_callback, this, _1));
...
private:
  ...
  void sensor_topic_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
  {
    ...
    while(((static_cast<unsigned long int>(std::chrono::high_resolution_clock::now().time_since_epoch().count())) -
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<SimDelaySpring>());
  rclcpp::shutdown();
  return 0;
}